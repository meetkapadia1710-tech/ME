...
export default function CaseStudy({ data }: { data: CaseStudyData }) {
  ...
  const { prev, next } = getAdjacent(data.slug);
    ...
    <div ref={rootRef}>
      ...
      <main className="px-6 pt-28 md:px-10 md:pt-32">
        ...
        <Section label="Approach">
          <p className="max-w-2xl font-mono text-sm leading-relaxed text-foreground/70 md:text-base">
            {data.approach}
          </p>
        </Section>
      ...
      <nav className="grid grid-cols-2 border-t border-foreground/10">
        ...
        >
          ...
          <span className="font-display text-2xl tracking-tight text-foreground/80 transition-colors group-hover:text-foreground md:text-4xl">
            {prev.name}
          </span>
        </Link>
        <Link
