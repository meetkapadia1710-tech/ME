...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  if (!data) return {};
  return {
    title: `${data.name} — Case Study — Meet Kapadia`,
...
export async function generateStaticParams() {
  const featuredProjects = await db.select({ slug: projects.slug }).from(projects).where(eq(projects.featured, true));
  return featuredProjects.map((p) => ({
    slug: p.slug,
  }));
}

export default async function WorkCaseStudy({ params }: { params: { slug: string } }) {
  const [dbData] = await db.select().from(projects).where(and(eq(projects.slug, params.slug), eq(projects.featured, true)));

  if (!dbData) {
    notFound();
  }

  // Get next and prev for case study navigation
  const allProjects = await db.select().from(projects).where(eq(projects.featured, true)).orderBy(desc(projects.createdAt));
